...
export default function About() {
  const stats = [
    { value: "45K+", label: "Instagram Followers", icon: <Users className="w-6 h-6 text-white mb-3" /> },
    { value: "13K+", label: "Orders Delivered", icon: <Heart className="w-6 h-6 text-white mb-3" /> },
    { value: "Worldwide", label: "Global Delivery", icon: <Globe className="w-6 h-6 text-white mb-3" /> },
    { value: "Lagos", label: "Established In", icon: <MapPin className="w-6 h-6 text-white mb-3" /> }
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <section className="bg-dark pt-32 pb-24 px-4 text-center border-b border-primary/20">
        ...
        >
          Meet <span className="text-primary italic">Sheedah</span>
        </motion.h1>
        <motion.p 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="w-full lg:w-1/2 space-y-6 text-dark/80 font-sans leading-relaxed text-lg"
          >
            <h2 className="text-3xl lg:text-4xl font-heading font-bold text-primary mb-8 border-b-4 border-secondary pb-2 block inline-block">
              Our Origin Story
            </h2>
            <p>
      ...
      {/* Section 3 — Stats Row */}
      <section className="py-16 bg-primary border-y border-white/10 relative z-10 w-full">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x-0 md:divide-x divide-y md:divide-y-0 divide-gold/20">
            {stats.map((stat, index) => (
              <motion.div 
                ...
                className={`flex flex-col items-center text-center p-4 ${index > 1 ? 'pt-8 md:pt-4' : ''} ${index === 1 || index === 3 ? 'border-t border-white/20 md:border-t-0' : ''}`}
              >
                {stat.icon}
                <span className="text-3xl lg:text-4xl font-heading font-bold text-white mb-2">{stat.value}</span>
                <span className="text-white font-sans uppercase tracking-widest text-xs font-semibold">{stat.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* Section 4 — Mission */}
      <section className="py-32 px-4 sm:px-6 lg:px-8 bg-lightbg text-center flex flex-col items-center justify-center relative overflow-hidden">
        <motion.div 
        ...
        >
          <span className="text-8xl absolute -top-10 -left-6 md:-left-12 opacity-20 text-secondary font-heading leading-none">&quot;</span>
          <p className="text-2xl md:text-4xl lg:text-5xl font-heading font-bold text-secondary leading-tight italic z-10 relative">
